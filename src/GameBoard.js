...
class GameBoard extends React.Component{
    ...
    let heroList = {"1":[new heroStruct(0, {"value": "250", "label": "Marisa: Crimson Flash"}), new heroStruct(1, {"value": "2", "label": "Alfonse: Prince of Askr"}), new heroStruct(2, {"value": "166", "label": "Innes: Flawless Form"}), new heroStruct(3), new heroStruct(4), new heroStruct(5)],
  ...
  dragOverBoard(ev){
    ...
    if (cellContent !==null && this.state.draggedHero.side !== cellContent.side && cellContent.type === "hero"){
      ...
      if ( this.state.draggedOver === null ||   this.state.draggedOver.position !== dropPosition) {
        ...
        //does not have the game state so could possibly cause issues
...
function calculateAuraStats(heroList, gameState){
  ...
  for (let team in heroList){ //loop both teams
    for (let hero of heroList[team]){ //loop through each member
      //These effects last for 1 turn which means they are reset at the start of the turnstart
